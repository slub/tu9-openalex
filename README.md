...
- `data/alliance_ca_oa_by_year.csv` — deduplicated alliance-level CA OA share by year and view (`single`/`hierarchy`/`consolidated`/`core_any`/`core_primary`): one OR-query per view across all nine universities' relevant OpenAlex ids, so a work matching more than one member institution is counted once. Global only — no per-institution slice.
